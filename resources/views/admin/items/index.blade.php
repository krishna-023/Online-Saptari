...
@component('components.breadcrumb')
    @slot('li_1') Items Management @endslot
    @slot('title') All Items @endslot
@endcomponent
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h2 class="text-primary mb-0" id="totalItems">{{ \App\Models\Item::count() }}</h2>
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h2 class="text-success mb-0" id="featuredItems">{{ \App\Models\Item::where('item_featured', 1)->count() }}</h2>
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h2 class="text-warning mb-0" id="totalCategories">{{ \App\Models\Category::count() }}</h2>
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h2 class="text-danger mb-0" id="monthItems">{{ \App\Models\Item::whereMonth('created_at', now()->month)->count() }}</h2>
            ...
            <div class="card-header d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                ...
                <div class="d-flex flex-column flex-md-row gap-3 w-100 w-md-auto">
                    ...
                    <div class="d-flex flex-wrap gap-2 justify-content-md-end">
                        <!-- Create Button -->
                        <a href="{{ route('item.add') }}" class="btn btn-primary">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                <li>
                                    <form action="{{ route('item.import') }}" method="POST" enctype="multipart/form-data" class="p-3" style="min-width: 300px;">
                                        @csrf
                                        <div class="mb-3">
                                        ...
                                        <div class="d-flex gap-2">
                                            ...
                                            <a href="{{ route('item.import.sample') }}" class="btn btn-outline-secondary btn-sm" title="Download Sample File">
                        ...
                        <!-- Export Button -->
                        <a href="{{ route('item.export') }}" class="btn btn-outline-primary">
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody id="itemsTableBody">
                            @forelse($items as $item)
                                <tr class="align-middle item-row" data-item-id="{{ $item->id }}">
                                    <td class="ps-4">
                                        <div class="form-check">
                                            <input class="form-check-input row-checkbox" type="checkbox" value="{{ $item->id }}" data-encrypted-id="{{ encrypt($item->id) }}">
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            <span class="fw-medium text-truncate" style="max-width: 200px;" title="{{ $item->title }}">
                                                {{ $item->title }}
                                            </span>
                                        </div>
                                    </td>
                                    <td>
                                        <span class="text-muted text-truncate d-inline-block" style="max-width: 200px;" title="{{ $item->subtitle }}">
                                            {{ $item->subtitle ?: '—' }}
                                        </span>
                                    </td>
                                    <td>
                                        <span class="text-truncate d-inline-block" style="max-width: 150px;" title="{{ $item->contacts->telephone ?? 'N/A' }}">
                                            {{ $item->contacts->telephone ?? '—' }}
                                        </span>
                                    </td>
                                    <td>
                                        <span class="text-truncate d-inline-block" style="max-width: 200px;" title="{{ $item->contacts->email ?? 'N/A' }}">
                                            {{ $item->contacts->email ?? '—' }}
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-end gap-1 action-buttons">
                                            <a href="{{ route('item.view', encrypt($item->id)) }}" class="btn btn-outline-primary btn-sm" title="View">
                                                <i class="ri-eye-line"></i>
                                            </a>
                                            <a href="{{ route('item.edit', encrypt($item->id)) }}" class="btn btn-outline-secondary btn-sm" title="Edit">
                                                <i class="ri-pencil-line"></i>
                                            </a>
                                            <form action="{{ route('item.destroy', encrypt($item->id)) }}" method="POST" class="d-inline delete-form">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-outline-danger btn-sm" title="Delete" data-item-title="{{ $item->title }}">
                    ...
                    <nav aria-label="Items pagination">
                        <ul class="pagination mb-0">
                            ...
                            @if($end < $last)
                                ...
                                @endif
                                <li class="page-item">
                                    <a class="page-link" href="{{ $items->url($last) }}">{{ $last }}</a>
                                </li>
                            @endif
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="filterForm" method="GET" action="{{ route('item.index') }}">
          <div class="row g-3">
            <div class="col-md-6">
            ...
            <div class="col-md-6">
              ...
              <select id="filter-featured" name="item_featured" class="form-select">
                ...
                <option value="1" {{ request('item_featured') == '1' ? 'selected' : '' }}>Featured Only</option>
                <option value="0" {{ request('item_featured') == '0' ? 'selected' : '' }}>Standard Only</option>
              </select>
            </div>
            <div class="col-12">
              <label for="filter-title" class="form-label">Title</label>
              <input type="text" id="filter-title" name="title" class="form-control" value="{{ request('title') }}" placeholder="Search by title...">
            </div>
            <div class="col-12">
              <label for="filter-subtitle" class="form-label">Subtitle</label>
              <input type="text" id="filter-subtitle" name="subtitle" class="form-control" value="{{ request('subtitle') }}" placeholder="Search by subtitle...">
            </div>
            <div class="col-12">
              <label for="filter-date" class="form-label">Created Date</label>
              <input type="date" id="filter-date" name="collection_date" class="form-control" value="{{ request('collection_date') }}">
            </div>
          </div>
        </form>
      </div>
...
$(document).ready(function() {
    ...
    function performServerSearch(searchTerm) {
        ...
        window.location.href = '{{ route('item.index') }}?' + urlParams.toString();
    }
    ...
    function deleteSelectedItems() {
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                return $.post('{{ route('item.deleteSelected') }}', {
                    _token: '{{ csrf_token() }}',
                    ids: selectedIds
                }).fail(() => {
    ...
    function autoFocusSearch() {
        @if(request('search'))
            $('#itemsearchbox').focus().val('{{ request('search') }}');
        @endif
    }
