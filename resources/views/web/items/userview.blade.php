...
<section class="modern-hero">
    ...
    <div class="container">
        <div class="hero-content">
            ...
            <!-- Category Badge -->
            @if($item->category)
            <a href="{{ route('category.items', $item->category->slug) }}" class="category-badge">
            ...
            <div class="hero-meta">
                ...
                <div class="meta-item">
                    ...
                    <span class="meta-value">{{ \Carbon\Carbon::parse($item->collection_date)->format('M d, Y') }}</span>
                </div>
                @endif
                ...
                <div class="meta-item">
                    ...
                    <span class="meta-value">{{ $item->item_locations }}</span>
                </div>
                @endif
                ...
                <div class="meta-item">
                    ...
                    <span class="meta-value">{{ $item->views ?? 0 }}</span>
                </div>
...
<div class="main-content">
        ...
        <div class="row g-4 mb-5">
            <div class="col-xl-3 col-md-6">
                <div class="modern-card text-center p-4 fade-up">
                    ...
                    <h5>Established</h5>
                    <p class="mb-0 text-muted">{{ $item->collection_date ? \Carbon\Carbon::parse($item->collection_date)->format('M d, Y') : 'N/A' }}</p>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="modern-card text-center p-4 fade-up">
                    ...
                    <h5>Location</h5>
                    <p class="mb-0 text-muted">{{ $item->item_locations ?? 'N/A' }}</p>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="modern-card text-center p-4 fade-up">
                    ...
                    <h5>Status</h5>
                    <p class="mb-0 text-muted">{{ $item->item_featured ? 'Featured' : 'Standard' }}</p>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="modern-card text-center p-4 fade-up">
                    ...
                    <h5>Author</h5>
                    <p class="mb-0 text-muted">{{ $item->author_first_name }} {{ $item->author_last_name }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="modern-tabs">
            <div class="tab-header">
                ...
                <button class="tab-button" data-tab="contact">
                    ...
                    Contact
                    <span class="badge">{{ $item->contacts ? '1' : '0' }}</span>
                </button>
    ...
    <span class="badge">{{ $galleryCount }}</span>
                ...
                @if($item->contacts && ($item->contacts->latitude || $item->contacts->address))
                ...
                @endif
                @if($item->socialIcons && $item->socialIcons->count() > 0)
                <button class="tab-button" data-tab="social">
                    <i class="fas fa-share-alt"></i>
                    Social
                    <span class="badge">{{ $item->socialIcons->count() }}</span>
                </button>
                @endif
            ...
            <div class="tab-content active" id="overview">
                <div class="content-section">
                    {!! $item->content ?? '<div class="text-center py-5"><i class="fas fa-info-circle fa-4x text-muted mb-4"></i><h4>No Content Available</h4><p class="text-muted">This item doesn\'t have any detailed content yet.</p></div>' !!}
                </div>

                @if($item->contacts)
                <div class="feature-grid mt-5">
                    @if($item->contacts->telephone)
                    <div class="feature-item" onclick="window.location.href='tel:{{ $item->contacts->telephone }}'">
                        <div class="feature-icon">
                            <i class="fas fa-phone"></i>
                        </div>
                        <div class="feature-content">
                            <h5 class="feature-title">Primary Phone</h5>
                            <p class="feature-description">{{ $item->contacts->telephone }}</p>
                        </div>
                    </div>
                    @endif

                    @if($item->contacts->email)
                    <div class="feature-item" onclick="window.location.href='mailto:{{ $item->contacts->email }}'">
                        <div class="feature-icon">
                            <i class="fas fa-envelope"></i>
                        </div>
                        <div class="feature-content">
                            <h5 class="feature-title">Email Address</h5>
                            <p class="feature-description">{{ $item->contacts->email }}</p>
                        </div>
                    </div>
                    @endif

                    @if($item->contacts->web)
                    <div class="feature-item" onclick="window.open('{{ $item->contacts->web }}', '_blank')">
                        <div class="feature-icon">
                    ...
                    <div class="feature-item" onclick="document.querySelector('[data-tab=\"location\"]').click()">
                        ...
                        <div class="feature-content">
                            ...
                            <p class="feature-description">{{ $item->contacts->address }}</p>
                        </div>
                    </div>
                    @endif
                </div>
                    ...
                    <div class="col-lg-6">
                        <div class="modern-card h-100 p-4">
                            ...
                            <div class="feature-grid">
                                ...
                                @if($item->contacts->phone1)
                                <div class="feature-item" onclick="window.location.href='tel:{{ $item->contacts->phone1 }}'">
                                    <div class="feature-icon">
                                        <i class="fas fa-mobile-alt"></i>
                                    </div>
                                    <div class="feature-content">
                                        <h5 class="feature-title">Additional Phone 1</h5>
                                        <p class="feature-description">{{ $item->contacts->phone1 }}</p>
                                    </div>
                                </div>
                                @endif

                                @if($item->contacts->phone2)
                                <div class="feature-item" onclick="window.location.href='tel:{{ $item->contacts->phone2 }}'">
                                    <div class="feature-icon">
                                        <i class="fas fa-phone-square-alt"></i>
                                    </div>
                                    <div class="feature-content">
                                        <h5 class="feature-title">Additional Phone 2</h5>
                                        <p class="feature-description">{{ $item->contacts->phone2 }}</p>
                                    </div>
                                </div>
                                @endif
...
<div class="tab-content" id="gallery">
    <div class="gallery-section">
        ...
        <div class="alert alert-info mb-4">
            ...
            Found {{ count($allGalleryImages) }} images
            ({{ count(array_filter($allGalleryImages, fn($img) => $img['type'] === 'individual')) }} individual records,
            {{ count(array_filter($allGalleryImages, fn($img) => $img['type'] === 'array')) }} from arrays)
        </div>

        <div class="gallery-grid" id="lightgallery">
            @foreach($allGalleryImages as $index => $imageData)
                <div class="gallery-item"
                     data-src="{{ $imageData['url'] }}"
                     data-sub-html="<h4>Image {{ $index + 1 }}</h4><p>Storage: {{ ucfirst($imageData['type']) }}</p>">

                    <img src="{{ $imageData['url'] }}"
                         alt="Gallery Image {{ $index + 1 }}"
                         loading="lazy"
                         onerror="this.src='{{ asset('admin/images/logo-sm.png') }}'">

                    <div class="gallery-overlay">
                        <div class="text-white">
                            <h6 class="mb-1">Image {{ $index + 1 }}</h6>
                            <small>
                                @if($imageData['type'] === 'individual')
                                    <i class="fas fa-database me-1" title="Individual Record"></i>
                                @elseif($imageData['type'] === 'array')
                                    <i class="fas fa-layer-group me-1" title="Array Storage"></i>
                                @else
                                    <i class="fas fa-download me-1" title="Local File"></i>
                                @endif
                                {{ ucfirst($imageData['type']) }} Storage
                            </small>
                        </div>
                    </div>

                    <div class="gallery-actions">
                        <button class="gallery-action-btn" onclick="event.stopPropagation(); window.open('{{ $imageData['url'] }}', '_blank')">
                            <i class="fas fa-external-link-alt"></i>
                        </button>
                        <button class="gallery-action-btn" onclick="event.stopPropagation(); downloadImage('{{ $imageData['url'] }}', 'image-{{ $index + 1 }}')">
        ...
        <div class="text-center mt-4">
            <p class="text-muted">
                ...
                Displaying {{ count($allGalleryImages) }} images from multiple storage methods
            </p>
        </div>
    </div>
</div>
    ...
    <div class="text-center py-5">
        ...
        @if(Auth::check() && Auth::user()->isAdmin())
            <div class="modern-card p-4 d-inline-block text-start">
                ...
                <p class="small text-muted mb-1">Total Gallery Records: {{ $item->galleries->count() }}</p>
                <p class="small text-muted mb-1">Individual Records: {{ $item->galleries->where('image_url', '!=', null)->count() }}</p>
                <p class="small text-muted mb-1">Array Records: {{ $item->galleries->where('gallery', '!=', null)->count() }}</p>
                <p class="small text-muted">Local Files: {{ $item->galleries->where('local_path', '!=', null)->count() }}</p>
            </div>
        @endif
                ...
                <div class="row mt-4">
                    ...
                    <div class="col-md-6">
                        <div class="modern-card p-3 text-center">
                            <small class="text-muted d-block">Latitude</small>
                            <strong class="fs-5">{{ $item->contacts->latitude }}</strong>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="modern-card p-3 text-center">
                            <small class="text-muted d-block">Longitude</small>
                            <strong class="fs-5">{{ $item->contacts->longitude }}</strong>
                        </div>
                    </div>
                    @endif
                </div>
        ...
        <div class="col-lg-4">
            <div class="modern-card p-4 h-100">
                ...
                <div class="d-grid gap-2">
                    ...
                    @endif
                    <button class="btn btn-outline-secondary" onclick="copyAddress('{{ $item->contacts->address }}')">
...
@endif
            ...
            <div class="tab-content" id="social">
                <div class="social-hub">
                    @foreach($item->socialIcons as $social)
                    <a href="{{ $social->socialIcons_url }}"
                       target="{{ $social->socialIconsOpenInNewWindow ? '_blank' : '_self' }}"
                       class="social-card {{ $social->socialIcons }}">
                        <i class="fab fa-{{ $social->socialIcons }} social-icon"></i>
                        <h5 class="mb-2">{{ ucfirst($social->socialIcons) }}</h5>
                        <p class="mb-0 opacity-90">Follow us on {{ ucfirst($social->socialIcons) }}</p>
                        <small class="opacity-75 mt-2 d-block">{{ Str::limit($social->socialIcons_url, 30) }}</small>
                    </a>
                    @endforeach
                </div>
            </div>
                ...
                <div class="opening-hours">
                    ...
                    <div class="timeline">
                        ...
                        @foreach($days as $day => $time)
                            @if($time && $time !== '-')
                            <div class="timeline-item {{ in_array(strtolower($time), ['closed', 'close', 'holiday']) ? 'closed' : '' }}">
                                <div class="timeline-day">{{ $day }}</div>
                                <div class="timeline-time">
                                    @if(in_array(strtolower($time), ['closed', 'close', 'holiday']))
                    ...
                    @if($item->openingTimes->openingHoursNote)
                    <div class="modern-card p-4 mt-4">
                        <div class="d-flex align-items-start gap-3">
                            <i class="fas fa-info-circle text-primary mt-1"></i>
                            <div>
                                <h6 class="mb-2">Additional Information</h6>
                                <p class="text-muted mb-0">{{ $item->openingTimes->openingHoursNote }}</p>
                            </div>
                        </div>
                    </div>
                    @endif
...
function initializeTabs() {
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            // Track tab view
            trackEvent('tab_view', { tab: tabId, item_id: {{ $item->id }} });
        });
    });
...
class MapUtils {
    static getTomTomKey() {
        ...
        return '{{ config("services.tomtom.key", "eV5ZiNE8kKeHSBqvzXL4ZbvN9D0JMZOf") }}';
    }
...
function initializeTomTomMap() {
    ...
    const latitude = {{ $item->contacts->latitude ?? '27.7172' }};
    const longitude = {{ $item->contacts->longitude ?? '85.3240' }};
    const address = `{{ $item->contacts->address ?? '' }}`;
    ...
    mapContainer.innerHTML = `
        <div style="text-align: center; padding: 40px 20px; background: #f8f9fa; border-radius: 8px; height: 100%; display: flex; flex-direction: column; justify-content: center; align-items: center;">
            ...
            <div class="d-flex gap-2 justify-content-center flex-wrap">
                <button onclick="MapUtils.openMapView({{ $item->contacts->latitude ?? 'null' }}, {{ $item->contacts->longitude ?? 'null' }})"
...
function getGoogleDirections() {
    const latitude = {{ $item->contacts->latitude ?? 'null' }};
    const longitude = {{ $item->contacts->longitude ?? 'null' }};
...
function downloadImage(url, filename) {
    fetch(url)
        ...
        .then(blob => {
            ...
            trackEvent('image_download', { item_id: {{ $item->id }} });
        })
